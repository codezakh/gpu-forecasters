...
def fused_ln_dual_matmul_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        w_mask = ((k_block_start + offs_k)[:, None] < K) & (
            offs_n_4way[None, :] < N_4way
        )
    ...
    if pid_n * BLOCK_SIZE_N < H:
        ...
        outg_ptrs = (
            OutOG_ptr + offs_m[:, None] * stride_og_m + offs_n_og[None, :] * stride_og_h
        )
    ...
    mask_ptrs = (
        ...
        + offs_h_chunk[None, :] * stride_mask_h
    )
    ...
    outl_ptrs = OutLeft_ptr + (
        offs_b_2d * stride_ol_bs
        + offs_h_2d * stride_ol_h
        + offs_s1_2d * stride_ol_s1
        + offs_s2_2d * stride_ol_s2
    )
    outr_ptrs_t = OutRight_ptr + (
        offs_b_2d * stride_or_t_bs
        + offs_h_2d * stride_or_t_h
        + offs_s2_2d * stride_or_t_s2
        + offs_s1_2d * stride_or_t_s1
    )
...
def bmm_coalesced_kernel(
    ...
    for k in range(0, tl.cdiv(s2, BLOCK_SIZE_K)):
        ...
        a_ptrs = left_ptrs_base + (
            offs_m[:, None] * stride_l_s1 + (k_start + offs_k[None, :]) * stride_l_s2
        )
        b_ptrs = right_ptrs_base + (
            (k_start + offs_k[:, None]) * stride_r_s2 + offs_n[None, :] * stride_r_s1
        )
    ...
    out_ptrs = (
        ...
        + pid_b * stride_o_bs
        + pid_h * stride_o_h
        + offs_m[:, None] * stride_o_s1
        + offs_n[None, :] * stride_o_s2
    )
...
def fused_final_kernel(
    ...
    in_ptr_base = (
        In_ptr + b * stride_in_bs + r * stride_in_s1_row + c * stride_in_s1_col
    )
    ...
    for k_offset in range(0, H, BLOCK_SIZE_K):
        ...
        in_chunk = tl.load(
            in_ptrs, mask=m_mask[:, None] & k_mask[None, :], other=0.0
        ).to(tl.float32)
    ...
    for k_offset in range(0, H, BLOCK_SIZE_K):
        ...
        gate_ptrs = (
            Gate_ptr + offs_m[:, None] * stride_gate_m + offs_k[None, :] * stride_gate_h
        )
        gate = tl.load(gate_ptrs, mask=m_mask[:, None] & k_mask[None, :], other=0.0)
        a_gated = a_norm * gate
        b_w = tl.load(
            proj_ptrs, mask=k_mask[:, None] & (offs_n[None, :] < D), other=0.0
        )
        acc += tl.dot(a_gated.to(b_w.dtype), b_w)

    offs_d = pid_n * BLOCK_SIZE_N + tl.arange(0, BLOCK_SIZE_N)
    out_ptr_base = (
        Out_ptr + b * stride_out_bs + r * stride_out_s1_row + c * stride_out_s1_col
    )
...
def compiledtrimul_fused_interleaved_final(
    ...
    grid_k1 = lambda meta: (
        ...
        * triton.cdiv(N_4way, meta["BLOCK_SIZE_N"]),
    )
    ...
    fused_ln_dual_matmul_kernel[grid_k1](
        ...
        left_final.stride(2),
        left_final.stride(3),
        right_final_t.stride(0),
        right_final_t.stride(1),
        right_final_t.stride(2),
        right_final_t.stride(3),
        og_mh.stride(0),
        og_mh.stride(1),
        mask_mh.stride(0),
        mask_mh.stride(1),
    ...
    grid_k2 = lambda meta: (
        triton.cdiv(s1, meta["BLOCK_SIZE_M"]) * triton.cdiv(s1, meta["BLOCK_SIZE_N"]),
        bs * H,
    )
    ...
    bmm_coalesced_kernel[grid_k2](
        ...
        right_final_t.stride(2),
        right_final_t.stride(3),
        bmm_out_tmp.stride(0),
        bmm_out_tmp.stride(1),
        bmm_out_tmp.stride(2),
        bmm_out_tmp.stride(3),
        **config_k2,
        num_warps=4,
        num_stages=3
    )
    ...
    grid_k3 = lambda meta: (
        triton.cdiv(M, meta["BLOCK_SIZE_M"]) * triton.cdiv(d, meta["BLOCK_SIZE_N"]),
    )
    ...
    fused_final_kernel[grid_k3](
        ...
        og_mh.stride(0),
        og_mh.stride(1),
        to_out_weight.stride(0),
        to_out_weight.stride(1),
        final_out.stride(0),
        final_out.stride(1),
        final_out.stride(2),
        final_out.stride(3),
...
def pack_w_4way_efficient(weights):
    ...
    H, K = WL.shape
    ws = (
        torch.stack([WL, WLG, WR, WRG], dim=0)
        .permute(1, 0, 2)
        .contiguous()
        .view(4 * H, K)
    )
...
def compiledtrimul(
    ...
    out_p = torch.matmul(
        left.to(torch.float16), right.to(torch.float16).transpose(-1, -2)
    )
    out_einsum_flat = out_p.permute(0, 2, 3, 1).reshape(bs * s1 * s1, h)
    normed = F.layer_norm(
        out_einsum_flat, (h,), to_out_norm_weight, to_out_norm_bias
    ).to(torch.float16)
