...
class GreedySearch:
    ...
    def _create_initial_checkpoint(self) -> GreedySearchCheckpoint:
        ...
        starter_result = self.config.scoring_provider.score_reference(
            self.config.starter_kernel_code
        )
        if starter_result.is_err():
            raise ValueError(
                f"Cannot initialize search: starter kernel scoring failed. "
                f"{starter_result.unwrap_err()}"
            )
        starter_evaluation = starter_result.unwrap()
    ...
    def _attempt_generate_mutations(
        ...
        return self.config.mutation_provider.generate_mutations(mutation_context)
    ...
    def _attempt_score_mutations(
        ...
        return self.config.scoring_provider.score_candidates(
            candidates, self.config.reference_kernel_code
        )
