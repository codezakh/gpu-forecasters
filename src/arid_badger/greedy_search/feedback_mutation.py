...
from .components import MutationContext, MutatedKernel
from .domain import KernelExecutionFeedback
...
def format_feedback_mutation_prompt(
    *,
    ...
    if feedback.kind == "compile_failed":
        ...
        raise ValueError(f"Unknown feedback kind: {feedback.kind}")

    return prompt


class FeedbackMutationFunction:
    ...
    def __call__(self, context: MutationContext) -> MutatedKernel:
        if context.previous_evaluation is None:
            raise ValueError(
                "FeedbackMutationFunction requires context.previous_evaluation."
            )
        ...
        prompt = format_feedback_mutation_prompt(
            ...
            previous_kernel_code=context.previous_kernel_code or "",
            feedback=feedback,
        )
        ...
        logger.info(
            "Feedback mutation response received from LLM (chars={length}).",
            length=len(content),
        )
