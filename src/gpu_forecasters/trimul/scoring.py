"""TriMul scoring pipeline.

Accepts a mutated-kernel source string, resolves its ``custom_kernel``
function via a synthetic-module shim (design note #3), runs the
correctness oracle, then times both candidate and reference in the
adaptive cuda.Event loop. Returns raw nanoseconds — the provider layer
computes speedup.

The heavy lifting (adaptive loop body) is ported from
``ttt-discover/.../eval.py`` ``_run_single_benchmark`` lines 201-258,
collapsed to single-case: we don't need the ``recheck`` branch or the
multi-case combination logic for this library.
"""
...
_TASK_SHIM = """\
# Synthetic stand-in for ttt-discover's task.py so that candidate sources
# containing `from task import input_t, output_t` resolve at import time.
from typing import Any, Dict, Tuple, TypeVar
import torch

input_t = TypeVar(
    "input_t",
    bound=Tuple[torch.Tensor, torch.Tensor, Dict[str, torch.Tensor], Dict],
)
...
def _loaded_candidate(
    ...
    Also drops ``task.py`` and ``utils.py`` shims into the same tmpdir
    so candidates that look like the upstream popcorn ``submission.py``
    template (``from task import input_t``) resolve without modification.
    ...
    """
    tmpdir = tempfile.mkdtemp(prefix="trimul-candidate-")
    try:
        ...
        # Evict any stale cached modules from a previous call.
        ...
        try:
            try:
                ...
                # Module import raised (NameError, ImportError, etc.).
                raise CandidateResolutionError(
        ...
        # Best-effort cleanup; Modal container lifetime is short anyway.
        import shutil
...
# ---------------------------------------------------------------------------
# Cloning helper (copied from ttt-discover eval.py _clone_data)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Adaptive timing loop (ported from eval.py _run_single_benchmark body)
# ---------------------------------------------------------------------------
...
def _adaptive_time_ns(
    ...
    *,
    ...
    Stops when either relative error of the mean drops below 0.1%, total
...
def score(
    ...
    *,
    ...
    Returns ``Ok(TriMulExecResult)`` on successful execution of the
    ...
    try:
        ...
        with _loaded_candidate(mutated_kernel_code) as custom_kernel:
            ...
            # Candidate is correct — time it. A fresh copy is used on each
            # invocation in case the kernel mutates its inputs in place.
            ...
            # Time the reference on the same data, same hardware.
        ...
        return Err(
            ScoringError(
                reason=f"TriMul scoring harness crashed: {type(exc).__name__}: {exc}",
